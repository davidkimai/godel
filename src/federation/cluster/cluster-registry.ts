...
export class ClusterRegistry extends EventEmitter {
  ...
  register(cluster: Cluster): Cluster {
    ...
    const fullCluster: Cluster = {
      ...
      capabilities: {
        maxAgents: cluster.capabilities?.maxAgents ?? 0,
        availableAgents: cluster.capabilities?.availableAgents ?? 0,
        activeAgents: cluster.capabilities?.activeAgents ?? 0,
        gpuEnabled: cluster.capabilities?.gpuEnabled ?? false,
        gpuTypes: cluster.capabilities?.gpuTypes ?? [],
        costPerHour: cluster.capabilities?.costPerHour ?? 0,
        latency: cluster.capabilities?.latency ?? 0,
        flags: cluster.capabilities?.flags ?? {},
      },
      metadata: {
        version: cluster.metadata?.version ?? 'unknown',
        provider: cluster.metadata?.provider ?? 'unknown',
        environment: cluster.metadata?.environment ?? 'unknown',
