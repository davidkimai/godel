...
export interface ChannelResponse {
  ...
  content: string;
  timestamp: Date;
  latency: number;           // ms from send to receive
...
export interface AggregationConfig {
  ...
  timeout: number;              // ms to wait for all responses
  minResponses: number;         // minimum responses needed
  maxResponses: number;         // maximum responses to accept
  requireConsensus: boolean;
  consensusThreshold: number;   // 0-1, agreement level required
...
class ContentAnalyzer {
  ...
  private static isFactual(sentence: string): boolean {
    // Contains numbers
    if (/\d/.test(sentence)) return true;
    // Contains dates
    if (/\b\d{4}-\d{2}-\d{2}\b|\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\b/i.test(sentence)) return true;
    // Contains specific identifiers
    if (/\b(?:id|uuid|key|token|error|success|failed|completed)\b/i.test(sentence)) return true;
    return false;
  }
...
class ConflictResolver {
  ...
  private static groupBySimilarity(responses: ChannelResponse[]): ChannelResponse[][] {
    const groups: ChannelResponse[][] = [];
    const threshold = 0.6;
  ...
  ): { selected: ChannelResponse[]; discarded: ChannelResponse[]; reason: string } {
  ...
  private static firstWins(responses: ChannelResponse[]) {
  ...
  private static lastWins(responses: ChannelResponse[]) {
  ...
  private static majorityVote(responses: ChannelResponse[]) {
    // Group by similarity
    const groups: ChannelResponse[][] = [];
  ...
  private static confidenceBased(
    ...
    const scored = responses.map(r => {
      ...
      for (const other of responses) {
        ...
        if (similarity > 0.7) agreementCount++;
      }
  ...
  private static calculateChannelWeight(config: ChannelConfig): number {
...
export class ResponseAggregator {
  private pendingAggregations = new Map<string, PendingAggregation>();
  private config: AggregationConfig;
  private channelConfigs: Map<string, ChannelConfig>;
  
  constructor(
    config: Partial<AggregationConfig> = {},
    channelConfigs: Map<string, ChannelConfig> = new Map()
  ) {
  ...
  startAggregation(requestId: string): AggregatedResponse {
    const aggregation: AggregatedResponse = {
      id: this.generateId(),
      requestId,
      status: 'pending',
      responses: [],
   ...
   */
  private finalize(requestId: string): AggregatedResponse {
    const pending = this.pendingAggregations.get(requestId);
    if (!pending) {
      throw new Error(`No pending aggregation for request ${requestId}`);
    }
    ...
    aggregation.confidence = this.calculateConfidence(
      aggregation.responses,
   ...
   */
  private mergeContent(responses: ChannelResponse[]): string {
    if (responses.length === 0) return '';
    if (responses.length === 1) return responses[0].content;
  ...
  private calculateConfidence(
    responses: ChannelResponse[],
    conflicts: Conflict[],
    resolution: { selected: ChannelResponse[]; reason: string }
  ): number {
...
// ============================================================================
// PENDING AGGREGATION (INTERNAL)
// ============================================================================

interface PendingAggregation {
  aggregation: AggregatedResponse;
...
export class LatencyOptimizer {
   ...
   */
  static calculateOptimalTimeout(
    channelConfigs: ChannelConfig[],
    confidenceLevel: number = 0.95
  ): number {
    if (channelConfigs.length === 0) return 5000;
