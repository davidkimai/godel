 ...
 */
export class ContextSizeCalculator {
  ...
  validate(files: ContextFile[]): ValidationResult {
    ...
    if (totalSize > this.limits.maxContextSize) {
      errors.push(
        `Context size (${this.formatSize(totalSize)}) exceeds limit (${this.formatSize(this.limits.maxContextSize)})`
      );
    }
    ...
    if (totalTokens > this.limits.maxContextWindow) {
      errors.push(
        `Token count (${totalTokens}) exceeds limit (${this.limits.maxContextWindow})`
      );
    }
    ...
    if (usagePercentage > this.limits.warningThreshold) {
      warnings.push(
        `Context usage at ${Math.round(usagePercentage * 100)}%, consider optimizing`
      );
    }
    ...
    for (const [type, count] of Object.entries(countByType)) {
      if (count > this.limits.maxFilesPerType) {
        warnings.push(
          `${type} context has ${count} files, max is ${this.limits.maxFilesPerType}`
        );
      }
    }
  ...
  getSizeSummary(files: ContextFile[]): object {
    ...
    return {
      ...
      sizeByType: Object.fromEntries(
        Object.entries(sizeByType).map(([k, v]) => [k, { bytes: v, formatted: this.formatSize(v) }])
      ),
      countByType,
  ...
  private groupByDirectory(files: ContextFile[]): Record<string, ContextFile[]> {
 ...
 */
export function calculateContextStats(files: ContextFile[]): object {
