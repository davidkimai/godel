...
class EventRepository {
    constructor() {
        ...
        this.cache = new cache_1.LRUCache({ maxSize: 100, defaultTTL: 60000 });
        // Cache for event lists by agent (very short TTL as events change frequently)
        this.agentCache = new cache_1.LRUCache({ maxSize: 30, defaultTTL: 5000 });
    }
    async create(data) {
        ...
        await db.run(`INSERT INTO events (id, timestamp, event_type, source, payload, agent_id, swarm_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [event.id, event.timestamp, event.type, event.source || null, event.payload || null,
    ...
    async findByAgentId(agentId, limit = 100) {
        ...
        const rows = await db.all('SELECT * FROM events WHERE agent_id = ? ORDER BY timestamp DESC LIMIT ?', [agentId, limit]);
    ...
    async getStats(timeWindowHours = 24) {
        ...
        const rows = await db.all('SELECT event_type as type, COUNT(*) as count FROM events WHERE timestamp >= ? GROUP BY event_type', [since]);
        const byType = {};
        let total = 0;
