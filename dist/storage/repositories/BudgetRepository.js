...
class BudgetRepository {
    async create(data) {
        ...
        await db.run(`INSERT INTO budgets (id, scope_type, scope_id, max_tokens, max_cost, 
        used_tokens, used_cost, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [budget.id, budget.scope_type, budget.scope_id, budget.max_tokens,
    ...
    async findByScope(scopeType, scopeId) {
        const db = await (0, sqlite_1.getDb)();
        return db.get('SELECT * FROM budgets WHERE scope_type = ? AND scope_id = ?', [scopeType, scopeId]);
    }
    async getUsage(id) {
    ...
    async addUsage(id, tokens, cost) {
        const db = await (0, sqlite_1.getDb)();
        await db.run(`UPDATE budgets 
       SET used_tokens = used_tokens + ?,
           used_cost = used_cost + ?
       WHERE id = ?`, [tokens, cost, id]);
    }
    /**
    ...
    async consumeBudget(id, tokens, cost) {
        ...
        return db.withTransaction(async () => {
            ...
            await db.run(`UPDATE budgets SET used_tokens = ?, used_cost = ? WHERE id = ?`, [newTokens, newCost, id]);
            return true;
        });
    }
    async isExceeded(id) {
