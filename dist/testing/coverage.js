...
exports.detectCoverageFormat = detectCoverageFormat;
exports.parseCoverage = parseCoverage;
exports.checkCoverageThresholds = checkCoverageThresholds;
exports.formatCoverageSummary = formatCoverageSummary;
 ...
 */
function detectCoverageFormat(cwd) {
 ...
 */
async function parseCoverage(cwd, framework) {
 ...
 */
async function parseIstanbulCoverage(cwd, framework) {
    const files = [];
 ...
 */
function parseIstanbulFileCoverage(coverage, filePath) {
 ...
 */
function parseLcovContent(content, baseDir) {
    const files = [];
 ...
 */
async function parseCoveragePy(cwd, framework) {
    const files = [];
 ...
 */
function parseCoverageXml(content, baseDir) {
    const files = [];
 ...
 * Parse gcov output
 */
async function parseGcovCoverage(cwd, framework) {
    const files = [];
 ...
 */
async function parseJacocoCoverage(cwd, framework) {
    const files = [];
    const jacocoReports = fs.readdirSync(cwd).filter(f => f.includes('jacoco') && f.endsWith('.xml'));
 ...
 */
function parseJacocoXml(content, baseDir) {
    const files = [];
    ...
    while ((packageMatch = packageRegex.exec(content)) !== null) {
        ...
        while ((sourceMatch = sourceRegex.exec(packageSection)) !== null) {
            ...
            while ((counterMatch = counterRegex.exec(sourceSection)) !== null) {
                ...
                const metric = { covered, total, percentage: 0 };
                if (total > 0) {
 ...
 */
function calculateAggregateMetrics(files) {
 ...
 */
function checkCoverageThresholds(metrics, thresholds) {
    const failures = [];
 ...
 */
function formatCoverageSummary(metrics) {
    const lines = [];
 ...
 */
function generateCoverageBadge(metrics, type = 'statements') {
