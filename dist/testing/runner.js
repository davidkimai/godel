...
exports.findAffectedTests = findAffectedTests;
exports.runIncrementalTests = runIncrementalTests;
        ...
        try {
            ...
            if (testScript) {
                ...
                if (testScript.includes('pytest'))
                    return 'pytest';
                if (testScript.includes('unittest'))
                    return 'unittest';
                if (testScript.includes('cargo'))
                    return 'cargo';
 ...
 */
async function discoverTests(cwd, pattern = '**/*.test.{ts,js,py,go,rs}', ignorePatterns = []) {
    ...
    for (const p of patterns) {
        const matches = await (0, glob_1.glob)(p, {
            cwd,
 ...
 */
function detectFrameworkByFile(filePath) {
 ...
 */
async function estimateTestCount(filePath) {
    try {
 ...
 */
function buildTestCommand(config, files) {
    const options = {
        cwd: config.pattern,
        stdio: 'pipe',
    ...
    switch (config.framework) {
        ...
        case 'cargo':
            args.push('test');
            if (files)
                args.push('--test', files[0].replace('.rs', ''));
            if (config.coverage)
                args.push('--coverage');
 ...
 */
async function runTests(config, eventEmitter) {
 ...
 */
async function parseTestOutput(output, framework) {
    const suites = [];
    switch (framework) {
        case 'jest':
        case 'vitest':
 ...
 */
function parseCargoOutput(output) {
    const suites = [];
    const lines = output.split('\n');
    let currentSuite = null;
    for (const line of lines) {
 ...
 */
async function getChangedFiles(cwd, since) {
    // Use git to get changed files
    const changedFiles = [];
    try {
        const gitCmd = (0, child_process_1.spawn)('git', ['diff', '--name-only', `--since=${since.toISOString()}`], {
            cwd,
            stdio: 'pipe'
        });
        const stdout = [];
 ...
 */
async function findAffectedTests(cwd, changedFiles) {
    const affectedTests = [];
    for (const changedFile of changedFiles) {
        // Look for test files that might test this file
        const testFileName = path.basename(changedFile.path).replace(/\.(ts|js|py|go|rs)$/, '');
 ...
 */
async function runIncrementalTests(config, since, eventEmitter) {
