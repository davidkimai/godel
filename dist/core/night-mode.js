...
exports.nightModeManager = exports.NightModeManager = void 0;
exports.isNightModeActive = isNightModeActive;
exports.getNightModeStatus = getNightModeStatus;
exports.recordHumanReturn = recordHumanReturn;
exports.canSpawnInNightMode = canSpawnInNightMode;
...
class NightModeManager {
    ...
    async enableNightMode() {
        ...
        logger_1.logger.info('night-mode', 'Night mode ENABLED', {
    ...
    async disableNightMode() {
        ...
        logger_1.logger.info('night-mode', 'Night mode DISABLED', {});
    }
    recordHumanActivity() {
        this.lastHumanActivity = new Date();
        if (this.enabled) {
    ...
    async generateMorningSummary(options) {
        ...
        logger_1.logger.info('night-mode', 'Morning summary generated', {
    ...
    generateRecommendations(options) {
        ...
        if (fixes > 0)
            recs.push(`${fixes} critical fixes applied`);
        if (incidents > 0)
            recs.push(`${incidents} health incidents - review logs`);
        if (spend > 20)
            recs.push(`High overnight spend: $${spend.toFixed(2)}`);
        if (recs.length === 0)
            recs.push('Quiet night - no issues detected');
        return recs;
    }
}
exports.NightModeManager = NightModeManager;
exports.nightModeManager = new NightModeManager();
function isNightModeActive() {
    return exports.nightModeManager.getStatus().enabled;
}
function getNightModeStatus() {
    const status = exports.nightModeManager.getStatus();
    const emoji = status.enabled ? '🌙' : '☀️';
    return `${emoji} ${status.isWithinNightHours ? 'Night' : 'Day'} Mode | Next: ${status.timeUntilNextTransition}`;
}
function recordHumanReturn() {
    exports.nightModeManager.recordHumanActivity();
}
function canSpawnInNightMode(isCritical) {
    const status = exports.nightModeManager.getStatus();
    if (!status.enabled)
        return true;
    if (isCritical && status.config.criticalFixesAllowed)
        return true;
    return false;
}
