...
exports.healthMonitor = exports.HealthMonitor = exports.DEFAULT_HEALTH_CHECKS = void 0;
...
async function checkOpenClawGateway() {
    ...
    try {
        ...
        const { GatewayClient } = await Promise.resolve().then(() => __importStar(require('../integrations/openclaw/GatewayClient')));
 ...
 */
async function checkAgentPool() {
    const start = Date.now();
    try {
        // Import AgentLifecycle dynamically
        const { getGlobalLifecycle } = await Promise.resolve().then(() => __importStar(require('./lifecycle')));
...
async function checkBudgetStatus() {
    const start = Date.now();
    try {
        const { budgetController, HARD_LIMITS } = await Promise.resolve().then(() => __importStar(require('./budget-controller')));
...
class HealthMonitor {
    constructor(customChecks = []) {
        ...
        for (const check of exports.DEFAULT_HEALTH_CHECKS) {
    ...
    registerCheck(check) {
        ...
        logger_1.logger.info('health-monitor', `Registered health check: ${check.name}`);
    }
    /**
    ...
    async runChecks(names) {
        ...
        const checksToRun = names
            ? names.map(n => this.checks.get(n)).filter((c) => c !== undefined)
            : Array.from(this.checks.values()).filter(c => c.enabled);
        const results = await Promise.all(checksToRun.map(async (check) => {
            try {
        ...
        for (const result of results) {
            if (result.status === 'unhealthy') {
                logger_1.logger.error('health-monitor', `Unhealthy check: ${result.name}`, {
                    message: result.message,
                    severity: result.severity,
                });
            }
            else if (result.status === 'degraded') {
                logger_1.logger.warn('health-monitor', `Degraded check: ${result.name}`, {
                    message: result.message,
                });
            }
        }
    ...
    async runCheck(name) {
        ...
        if (!check) {
            logger_1.logger.warn('health-monitor', `Unknown health check: ${name}`);
            return null;
        }
        try {
            return await check.check();
...
async function isSystemHealthy() {
    return exports.healthMonitor.isHealthy();
}
/**
...
function getQuickStatus() {
    const report = exports.healthMonitor.getLastReport();
    if (!report)
