...
exports.DependencyGraphBuilder = exports.DependencyAnalyzer = void 0;
...
function createParser(language) {
    return {
        inferDependencies: (_filePath, _content) => {
            const detectedLang = language || (0, parser_1.detectLanguage)(_filePath);
            const imports = (0, parser_1.parseImports)(_content, detectedLang);
...
class DependencyAnalyzer {
    constructor(dependencies, contents, parser) {
    ...
    getTransitiveDependencies(filePath) {
        ...
        const stack = [this.normalizePath(filePath)];
    ...
    calculateDepth(filePath, visited) {
...
class DependencyGraphBuilder {
     ...
     */
    buildChainFrom(node, graph, visited) {
        if (visited.has(node)) {
            return [];
        }
        visited.add(node);
 ...
 */
function buildDependencyGraph(filePaths, contents, language) {
 ...
 */
function detectCycles(filePaths, contents, language) {
 ...
 */
function getDependencies(filePath, filePaths, contents, language) {
    const graph = buildDependencyGraph(filePaths, contents, language);
    const analyzer = new DependencyAnalyzer(graph, contents);
 ...
 */
function getDependents(filePath, filePaths, contents, language) {
    const graph = buildDependencyGraph(filePaths, contents, language);
    const analyzer = new DependencyAnalyzer(graph, contents);
 ...
 */
function getFileDependencies(files, filePath) {
    const builder = new DependencyGraphBuilder();
 ...
 */
function analyzeDependencyHealth(files) {
    const builder = new DependencyGraphBuilder();
    for (const f of files) {
        if (f.imports) {
            builder.addFile(f.path, undefined, { imports: f.imports });
