...
exports.ImprovementStore = void 0;
exports.getImprovementStore = getImprovementStore;
exports.resetImprovementStore = resetImprovementStore;
// ============================================================================
// Improvement Store
// ============================================================================
class ImprovementStore {
    ...
    async registerOptimizationPatterns() {
        const patterns = [
            {
      ...
      `, pattern.patternId, pattern.name, pattern.description, pattern.query, JSON.stringify(pattern.parameters), pattern.avgExecutionTimeMs);
        }
    }
    // ========================================================================
    // Store Operations
    ...
    `, id, entry.timestamp.toISOString(), entry.area, entry.strategy, entry.success ? 1 : 0, entry.confidence, entry.budgetUsed, entry.durationMs, entry.changes, entry.metrics.testCoverageDelta ?? null, entry.metrics.bugsFixed ?? null, entry.metrics.performanceImprovement ?? null, entry.metrics.codeQualityScore ?? null, entry.metrics.documentationCoverage ?? null, entry.metrics.linesAdded ?? null, entry.metrics.linesRemoved ?? null, entry.metrics.filesChanged ?? null, entry.context.swarmId, entry.context.agentCount, entry.context.modelUsed, JSON.stringify(entry.context.toolsUsed), entry.context.filesAffected ? JSON.stringify(entry.context.filesAffected) : null, entry.context.commitHash ?? null, entry.errorDetails ?? null, JSON.stringify(entry.tags));
    ...
    async updateStrategyEffectiveness(strategy, area, entry) {
        // Get existing stats
    ...
    `, strategy, area, stats.totalAttempts, stats.successes, stats.failures, stats.successRate, stats.avgBudgetUsed, stats.avgDurationMs, stats.avgChanges, stats.costPerSuccess, stats.timePerSuccess, stats.firstUsed.toISOString(), stats.lastUsed.toISOString(), stats.effectivenessScore);
    }
    calculateEffectivenessScore(successRate, avgBudget, avgDuration) {
     ...
     */
    async getStrategyEffectiveness(area, minAttempts = 5) {
     ...
     */
    async getTimeSeries(period, area, limit = 30) {
     ...
     */
    async executePattern(patternId, parameters) {
...
exports.ImprovementStore = ImprovementStore;
// ============================================================================
// Singleton Instance
// ============================================================================
let globalImprovementStore = null;
function getImprovementStore(storage, config) {
...
function resetImprovementStore() {
    globalImprovementStore = null;
}
// ImprovementStore already exported above
//# sourceMappingURL=ImprovementStore.js.map