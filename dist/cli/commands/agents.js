...
function registerAgentsCommand(program) {
    ...
    agents
        ...
        try {
            ...
            if (options.dryRun) {
                ...
                if (options.budget)
                    console.log(`   Budget: $${options.budget} USD`);
                return;
            }
    ...
    agents
        ...
        try {
            ...
            if (state.status === agent_1.AgentStatus.PAUSED) {
                console.log(`⏸️  Agent ${agentId.slice(0, 16)}... is already paused`);
                return;
            }
            if (state.status !== agent_1.AgentStatus.RUNNING) {
    ...
    agents
        ...
        try {
            ...
            if (state.status === agent_1.AgentStatus.KILLED || state.status === agent_1.AgentStatus.COMPLETED) {
    ...
    agents
        ...
        try {
            ...
            if (state.pausedAt)
                console.log(`     Paused:     ${state.pausedAt.toISOString()}`);
            if (state.resumedAt)
                console.log(`     Resumed:    ${state.resumedAt.toISOString()}`);
    ...
    agents
        ...
        try {
            ...
            if (state.status !== agent_1.AgentStatus.FAILED && state.status !== agent_1.AgentStatus.KILLED) {
