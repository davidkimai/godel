...
exports.FileSandbox = void 0;
exports.getFileSandbox = getFileSandbox;
...
class FileSandbox extends events_1.EventEmitter {
    constructor(config = {}) {
        super();
     ...
     */
    createContext(agentId, swarmId) {
        const baseDirectory = path.join(this.config.allowedDirectories[0], 'swarms', swarmId, 'agents', agentId);
     ...
     */
    checkOperation(agentId, swarmId, operation, filePath) {
     ...
     */
    checkFileSize(agentId, swarmId, size) {
     ...
     */
    checkExecutionTime(agentId, swarmId) {
        const contextKey = this.getContextKey(agentId, swarmId);
        const context = this.contexts.get(contextKey);
        if (!context) {
     ...
     */
    getSandboxPath(agentId, swarmId, relativePath) {
        const contextKey = this.getContextKey(agentId, swarmId);
        let context = this.contexts.get(contextKey);
        if (!context) {
            context = this.createContext(agentId, swarmId);
     ...
     * Record operation
     */
    recordOperation(context, type, path, success, error) {
        const operation = {
            type,
            path,
            timestamp: new Date(),
     ...
     * Record violation
     */
    recordViolation(context, type, path, success, error) {
        const operation = {
            type,
            path,
            timestamp: new Date(),
