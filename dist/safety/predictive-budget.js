...
exports.getPredictiveBudget = getPredictiveBudget;
exports.createPredictiveBudget = createPredictiveBudget;
const events_1 = require("events");
class PredictiveBudget extends events_1.EventEmitter {
    constructor(config = {}) {
        super();
        this.usageHistory = [];
        this.alerts = [];
    ...
    detectAnomalies() {
        ...
        const stdDev = Math.sqrt(costs.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / costs.length);
    ...
    getOptimizations() {
        ...
        const avgTokens = recentRequests.reduce((sum, r) => sum + r.inputTokens + r.outputTokens, 0) / recentRequests.length;
        if (avgTokens > 5000) {
            suggestions.push({
                id: 'prompt-1',
                category: 'prompt',
    ...
    checkBudgetStatus() {
        ...
        if (metrics.status === 'warning' && !this.hasRecentAlert('warning')) {
            this.addAlert('warning', `Budget usage at ${metrics.percentageUsed.toFixed(1)}%. Consider optimizing.`, 'percentageUsed', metrics.percentageUsed, this.config.warningThreshold * 100);
        }
        // Critical alert
        if (metrics.status === 'critical' && !this.hasRecentAlert('critical')) {
            this.addAlert('critical', `CRITICAL: Budget at ${metrics.percentageUsed.toFixed(1)}%. Immediate action required!`, 'percentageUsed', metrics.percentageUsed, this.config.criticalThreshold * 100);
        }
        // Optimization alert
        if (metrics.percentageUsed > 50 && this.usageHistory.length > 10) {
            ...
            if (optimizations.length > 0 && !this.hasRecentAlert('optimization')) {
                ...
                this.addAlert('optimization', `Cost optimization: ${topOpt.description}`, 'suggestion', topOpt.estimatedSavings);
            }
        }
    }
    /**
 ...
 */
let instance = null;
function getPredictiveBudget() {
    if (!instance) {
        instance = new PredictiveBudget();
    }
    return instance;
}
function createPredictiveBudget(config) {
    return new PredictiveBudget(config);
}
//# sourceMappingURL=predictive-budget.js.map