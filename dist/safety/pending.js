...
exports.getApprovalDetails = getApprovalDetails;
exports.getRequestHistory = getRequestHistory;
exports.filterByAgent = filterByAgent;
exports.filterByStatus = filterByStatus;
exports.filterByRisk = filterByRisk;
exports.filterByOperationType = filterByOperationType;
exports.findExpiredRequests = findExpiredRequests;
exports.findStaleRequests = findStaleRequests;
exports.getStats = getStats;
exports.formatListForDisplay = formatListForDisplay;
exports.formatDetailsForDisplay = formatDetailsForDisplay;
exports.clearAllRequests = clearAllRequests;
...
function addToQueue(request) {
    ...
    utils_1.logger.debug('Added request to pending queue', {
...
function removeFromQueue(requestId) {
    ...
    utils_1.logger.debug('Removed request from pending queue', { requestId });
    return request;
}
function getFromQueue(requestId) {
...
function listPending(options = {}) {
    ...
    // Filter by agent ID
    if (options.agentId) {
        requests = requests.filter(r => r.requestingAgent.agentId === options.agentId ||
...
// Get Details
// ============================================================================
function getApprovalDetails(requestId, includeContext = false) {
...
function findExpiredRequests() {
    return Array.from(allRequests.values()).filter(r => r.status === 'pending' && r.expiresAt && new Date() > r.expiresAt);
}
function findStaleRequests(olderThanMinutes) {
    const cutoff = new Date(Date.now() - olderThanMinutes * 60 * 1000);
    return Array.from(allRequests.values()).filter(r => r.status === 'pending' && r.createdAt < cutoff);
}
function getStats() {
    ...
    if (completed.length > 0) {
        const totalTime = completed.reduce((sum, r) => sum + (r.decision.decidedAt.getTime() - r.createdAt.getTime()), 0);
...
// Format for Display
// ============================================================================
function formatListForDisplay(requests, format = 'table') {
    if (format === 'json') {
        return JSON.stringify(requests.map(approval_1.formatApprovalForDisplay), null, 2);
    }
...
function formatDetailsForDisplay(request) {
    const lines = [];
    const sep = '═'.repeat(70);
    ...
    if (request.task) {
        ...
        if (request.task.taskTitle)
            lines.push(`Title:       ${request.task.taskTitle}`);
        lines.push('');
    }
    if (request.decision) {
        lines.push('DECISION');
...
function clearAllRequests() {
    pendingQueue.clear();
    allRequests.clear();
    utils_1.logger.info('All approval requests cleared');
}
