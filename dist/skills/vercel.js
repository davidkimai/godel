...
exports.VercelSkillsClient = exports.DEFAULT_VERCEL_SKILLS_CONFIG = void 0;
exports.getGlobalVercelSkillsClient = getGlobalVercelSkillsClient;
exports.resetGlobalVercelSkillsClient = resetGlobalVercelSkillsClient;
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
const logger_1 = require("../utils/logger");
const types_1 = require("./types");
exports.DEFAULT_VERCEL_SKILLS_CONFIG = {
...
// Vercel Skills Client
// ============================================================================
class VercelSkillsClient {
    constructor(config) {
        this.cache = new Map();
        this.config = { ...exports.DEFAULT_VERCEL_SKILLS_CONFIG, ...config };
        this.cacheTtl = 5 * 60 * 1000; // 5 minutes
        logger_1.logger.info(`[VercelSkillsClient] Initialized`);
    }
    // ============================================================================
    // Configuration
    // ============================================================================
    ...
    updateConfig(config) {
        ...
        logger_1.logger.debug('[VercelSkillsClient] Configuration updated');
    }
    /**
    ...
    async search(params) {
        ...
        if (cached && Date.now() - cached.timestamp < this.cacheTtl) {
            logger_1.logger.debug('[VercelSkillsClient] Returning cached search results');
            const result = cached.data;
            return { ...result, queryTimeMs: Date.now() - startTime };
        }
        logger_1.logger.debug(`[VercelSkillsClient] Searching for: ${params.query}`);
        try {
            ...
            logger_1.logger.error('[VercelSkillsClient] Search failed:', error);
            throw new types_1.UnifiedSkillError('NETWORK_ERROR', `Failed to search Vercel skills: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Search npm registry
     */
    async searchNpm(params) {
    ...
    async searchSkillsSh(params) {
        try {
            ...
            if (!response.ok) {
                ...
                logger_1.logger.debug('[VercelSkillsClient] skills.sh API not available, using npm only');
                return [];
            }
            const skills = await response.json();
            // Filter by query if provided
            const filtered = params.query
                ? skills.filter(s => s.name.toLowerCase().includes(params.query.toLowerCase()) ||
            ...
            logger_1.logger.debug('[VercelSkillsClient] skills.sh search failed:', error);
            return [];
        }
    }
    /**
     ...
     */
    mergeSearchResults(npmResults, skillsShResults, params) {
        const seen = new Set();
        const merged = [];
    ...
    async fetchSkill(slug) {
        ...
        logger_1.logger.debug(`[VercelSkillsClient] Fetching skill: ${slug}`);
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    throw new types_1.SkillNotFoundError(slug, 'vercel');
                }
                throw new Error(`NPM registry returned ${response.status}`);
            }
            ...
            logger_1.logger.error(`[VercelSkillsClient] Failed to fetch skill ${slug}:`, error);
    ...
    async isInstalled(slug) {
        try {
            ...
            logger_1.logger.debug(`[VercelSkillsClient] Error checking installation: ${error}`);
            return { installed: false };
        }
    }
    /**
    ...
    async install(slug, options = {}) {
        logger_1.logger.info(`[VercelSkillsClient] Installing ${slug}`);
        try {
            // Check if already installed
            const existing = await this.isInstalled(slug);
            if (existing.installed && !options.force) {
                throw new types_1.UnifiedSkillError('SKILL_ALREADY_INSTALLED', `Skill ${slug}@${existing.version} is already installed. Use --force to reinstall.`);
            }
            ...
            // Install via npm
            const installPath = path.join(options.targetDir || this.getSkillsDirectory(), slug);
            ...
            logger_1.logger.debug(`[VercelSkillsClient] Running: ${npmCommand}`);
            const { stdout, stderr } = await execAsync(npmCommand, {
                timeout: this.config.timeout,
                cwd: this.config.workdir,
            });
            if (stderr && !stderr.includes('WARN')) {
                logger_1.logger.warn(`[VercelSkillsClient] npm install warnings: ${stderr}`);
            }
            logger_1.logger.debug(`[VercelSkillsClient] npm output: ${stdout}`);
            // Update lockfile
            ...
            logger_1.logger.error(`[VercelSkillsClient] Installation failed for ${slug}:`, error);
            return {
                success: false,
    ...
    async uninstall(slug) {
        logger_1.logger.info(`[VercelSkillsClient] Uninstalling ${slug}`);
        ...
        try {
            ...
            // Update lockfile
            await this.removeFromLockfile(slug);
            logger_1.logger.info(`[VercelSkillsClient] Uninstalled ${slug}`);
        }
        catch (error) {
            logger_1.logger.error(`[VercelSkillsClient] Failed to uninstall ${slug}:`, error);
            throw new types_1.UnifiedSkillError('DOWNLOAD_FAILED', `Failed to uninstall ${slug}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
        ...
        try {
            ...
            for (const entry of lockfile.skills) {
                try {
                    ...
                    logger_1.logger.warn(`[VercelSkillsClient] Failed to fetch metadata for ${entry.slug}:`, error);
                }
            }
            return skills;
        }
        catch (error) {
            logger_1.logger.debug(`[VercelSkillsClient] Error listing installed skills: ${error}`);
            return [];
        }
    }
    // ============================================================================
     ...
     */
    npmPackageToUnifiedMetadata(pkg) {
        const slug = pkg.name;
     ...
     */
    skillsShToUnifiedMetadata(skill) {
...
exports.VercelSkillsClient = VercelSkillsClient;
// ============================================================================
// Singleton
// ============================================================================
let globalClient = null;
function getGlobalVercelSkillsClient(config) {
    if (!globalClient) {
...
function resetGlobalVercelSkillsClient() {
    globalClient = null;
}
exports.default = VercelSkillsClient;
