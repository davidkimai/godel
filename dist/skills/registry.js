...
exports.getGlobalSkillRegistry = getGlobalSkillRegistry;
exports.resetGlobalSkillRegistry = resetGlobalSkillRegistry;
const path = __importStar(require("path"));
const logger_1 = require("../utils/logger");
const clawhub_1 = require("./clawhub");
...
class UnifiedSkillRegistry {
    constructor(config) {
        this.cache = new Map();
        this.config = { ...types_1.DEFAULT_UNIFIED_REGISTRY_CONFIG, ...config };
        // Initialize adapters
        this.clawhub = (0, clawhub_1.getGlobalClawHubAdapter)({
        ...
        this.vercel = (0, vercel_1.getGlobalVercelSkillsClient)({
        ...
        logger_1.logger.info('[UnifiedSkillRegistry] Initialized');
    }
    // ============================================================================
    // Configuration
    // ============================================================================
    ...
    updateConfig(config) {
        ...
        logger_1.logger.debug('[UnifiedSkillRegistry] Configuration updated');
    }
    /**
    ...
    async search(params) {
        ...
        if (cached && Date.now() - cached.timestamp < this.config.cacheTtl) {
            logger_1.logger.debug('[UnifiedSkillRegistry] Returning cached search results');
            const result = cached.data;
            return { ...result, queryTimeMs: Date.now() - startTime };
        }
        logger_1.logger.debug(`[UnifiedSkillRegistry] Searching for: ${params.query}`);
        ...
        const searchPromises = sourcesToSearch.map(async (source) => {
            try {
                ...
                logger_1.logger.warn(`[UnifiedSkillRegistry] Search failed for ${source}:`, error);
        ...
        // Combine results
        let allSkills = [];
        let totalBySource = { clawhub: 0, vercel: 0 };
    ...
    async fetchSkill(skillId) {
        ...
        throw new types_1.SkillNotFoundError(slug);
    }
    // ============================================================================
    // Installation
    // ============================================================================
    ...
    async install(skillId, options = {}) {
        ...
        throw new types_1.AmbiguousSkillError(slug, matchingSources);
    }
    /**
     * Install from a specific source
     */
    async installFromSource(source, slug, options) {
        switch (source) {
            case 'clawhub':
    ...
    async uninstall(skillId) {
        ...
        const entry = unifiedLockfile.skills.find(s => {
            if (s.id === skillId)
                return true;
            if (source && s.source === source && s.slug === slug)
                return true;
            if (!source && s.slug === slug)
                return true;
            return false;
        });
        if (!entry) {
            throw new types_1.UnifiedSkillError('SKILL_NOT_FOUND', `Skill ${skillId} is not installed`);
        }
    ...
    async listInstalled() {
        ...
        for (const entry of unifiedLockfile.skills) {
            try {
                ...
                logger_1.logger.warn(`[UnifiedSkillRegistry] Failed to get info for ${entry.id}:`, error);
            }
        }
        return skills;
    }
    ...
    async saveUnifiedLockfile(lockfile) {
        const fs = await Promise.resolve().then(() => __importStar(require('fs/promises')));
    ...
    async addToUnifiedLockfile(entry) {
    ...
    async removeFromUnifiedLockfile(id) {
        const lockfile = await this.loadUnifiedLockfile();
    ...
    parseSkillId(skillId) {
        ...
        if (colonIndex > 0) {
            const sourcePart = skillId.substring(0, colonIndex);
    ...
    clearCache() {
        this.cache.clear();
        logger_1.logger.debug('[UnifiedSkillRegistry] Cache cleared');
    }
}
exports.UnifiedSkillRegistry = UnifiedSkillRegistry;
// ============================================================================
// Singleton
// ============================================================================
let globalRegistry = null;
function getGlobalSkillRegistry(config) {
    if (!globalRegistry) {
...
function resetGlobalSkillRegistry() {
    globalRegistry = null;
}
exports.default = UnifiedSkillRegistry;
