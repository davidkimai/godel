...
describe('Quality Gates Module', () => {
    describe('Score Calculations', () => {
        describe('calculateLintScore', () => {
            it('should return 1.0 for no issues', () => {
                ...
                const score = (0, gates_1.calculateLintScore)(results);
                expect(score).toBe(1);
            });
            it('should reduce score based on errors', () => {
                const summary = createLintSummary(5, 0, 0);
                const score = (0, gates_1.calculateLintScore)(summary.results);
            ...
            it('should weight errors more than warnings', () => {
                ...
                const errorsScore = (0, gates_1.calculateLintScore)(errorsOnly.results);
                const warningsScore = (0, gates_1.calculateLintScore)(warningsOnly.results);
        ...
        describe('calculateTypeScore', () => {
            it('should return 1.0 for no issues', () => {
                const score = (0, gates_1.calculateTypeScore)(0, 0);
                expect(score).toBe(1);
            });
            it('should penalize errors heavily', () => {
                const score = (0, gates_1.calculateTypeScore)(5, 0);
                expect(score).toBeLessThan(0.6);
            });
            it('should penalize warnings moderately', () => {
                const score = (0, gates_1.calculateTypeScore)(0, 10);
                expect(score).toBeLessThan(1);
                expect(score).toBeGreaterThan(0.8);
            });
            it('should have minimum score of 0 for errors', () => {
                const score = (0, gates_1.calculateTypeScore)(20, 0);
                expect(score).toBeGreaterThanOrEqual(0);
            });
        });
        describe('calculateSecurityScore', () => {
            it('should return 1.0 for no vulnerabilities', () => {
                const score = (0, gates_1.calculateSecurityScore)({ critical: 0, high: 0, medium: 0, low: 0 });
                expect(score).toBe(1);
            });
            it('should penalize critical vulnerabilities heavily', () => {
                const score = (0, gates_1.calculateSecurityScore)({ critical: 1, high: 1, medium: 0, low: 0 });
                expect(score).toBe(0);
            });
            it('should penalize high vulnerabilities heavily', () => {
                const score = (0, gates_1.calculateSecurityScore)({ critical: 0, high: 5, medium: 0, low: 0 });
                expect(score).toBe(0);
            });
            it('should penalize medium vulnerabilities moderately', () => {
                const score = (0, gates_1.calculateSecurityScore)({ critical: 0, high: 0, medium: 10, low: 0 });
                expect(score).toBe(0.5);
            });
            it('should penalize low vulnerabilities lightly', () => {
                const score = (0, gates_1.calculateSecurityScore)({ critical: 0, high: 0, medium: 0, low: 100 });
                expect(score).toBe(0);
            });
        });
    ...
    describe('Default Gates', () => {
        it('should have lint gate', () => {
            expect(gates_1.DEFAULT_GATES.lint).toBeDefined();
            expect(gates_1.DEFAULT_GATES.lint.type).toBe('lint');
            expect(gates_1.DEFAULT_GATES.lint.criteria.length).toBeGreaterThan(0);
        });
        it('should have types gate', () => {
            expect(gates_1.DEFAULT_GATES.types).toBeDefined();
            expect(gates_1.DEFAULT_GATES.types.type).toBe('types');
        });
        it('should have security gate', () => {
            expect(gates_1.DEFAULT_GATES.security).toBeDefined();
            expect(gates_1.DEFAULT_GATES.security.type).toBe('security');
            expect(gates_1.DEFAULT_GATES.security.autoRetry).toBe(false);
        });
        it('should have full gate', () => {
            expect(gates_1.DEFAULT_GATES.full).toBeDefined();
            expect(gates_1.DEFAULT_GATES.full.type).toBe('multi');
            expect(gates_1.DEFAULT_GATES.full.criteria.length).toBeGreaterThan(3);
        });
    });
    ...
    describe('Gate Result Formatting', () => {
        it('should format result as JSON', () => {
            ...
            const output = (0, gates_1.formatGateResult)(result, { format: 'json' });
            expect(output).toContain('"passed"');
            expect(output).toContain('"score"');
        });
        it('should format result as table', () => {
            const result = createPassingGateResult();
            const output = (0, gates_1.formatGateResult)(result, { format: 'table' });
        ...
        it('should format result as summary', () => {
            const result = createPassingGateResult();
            const output = (0, gates_1.formatGateResult)(result, { format: 'summary' });
            expect(output).toContain('PASSED');
        });
    });
});
// Helper functions for creating test data
function createLintSummary(errors, warnings, info) {
    const issues = [
...
function createPassingGateResult() {
    return {
        gate: gates_1.DEFAULT_GATES.lint,
        passed: true,
        score: 0.95,
        criterionScores: [
