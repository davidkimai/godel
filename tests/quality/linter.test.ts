...
import {
  ...
  runLinters,
  lintAgentCodebase
} from '../../src/quality/linter';
import { LintIssue, LintResult, SeverityLevel } from '../../src/quality/types';
...
describe('Linter Module', () => {
  describe('runESLint', () => {
    it('should return a valid LintResult structure', async () => {
      ...
      const result = await runESLint('/nonexistent', ['**/*.ts']);
  ...
  describe('runPrettier', () => {
    it('should return a valid LintResult structure', async () => {
      const result = await runPrettier('/nonexistent', ['**/*.ts']);
  ...
  describe('runPylint', () => {
    it('should return a valid LintResult structure', async () => {
      const result = await runPylint('/nonexistent', ['**/*.py']);
  ...
  describe('runMyPy', () => {
    it('should return type check result', async () => {
      const result = await runMyPy('/nonexistent', ['**/*.py']);
  ...
  describe('runCargoCheck', () => {
    it('should return a valid LintResult structure', async () => {
      const result = await runCargoCheck('/nonexistent');
  ...
  describe('runGolangciLint', () => {
    it('should return a valid LintResult structure', async () => {
      const result = await runGolangciLint('/nonexistent', ['**/*.go']);
  ...
  describe('runTypeScriptCheck', () => {
    it('should return type check result', async () => {
      const result = await runTypeScriptCheck('/nonexistent');
  ...
  describe('runLinters', () => {
    it('should return array of lint results', async () => {
      const results = await runLinters({
        cwd: '/nonexistent',
        language: 'all',
    ...
    it('should run language-specific linters', async () => {
      const tsResults = await runLinters({
        cwd: '/nonexistent',
        language: 'typescript',
        includePrettier: false,
        includeTypes: false
...
function createIssue(severity: SeverityLevel, file: string): LintIssue {
  return {
    id: `${file}:1:1`,
    rule: 'test-rule',
    category: 'style',
