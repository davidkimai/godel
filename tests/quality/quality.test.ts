...
describe('Score Calculation', () => {
  describe('calculateLintScore', () => {
    it('should return 1.0 for no issues', () => {
      ...
      expect(calculateLintScore(results)).toBe(1.0);
    });
    ...
    it('should penalize for errors', () => {
      ...
      expect(calculateLintScore(results)).toBe(0.25);
    });
    ...
    it('should penalize less for warnings', () => {
      ...
      expect(calculateLintScore(results)).toBe(0.8);
    });
    ...
    it('should combine errors and warnings', () => {
      ...
      expect(calculateLintScore(results)).toBe(0.45);
    });
  });

  describe('calculateTypeScore', () => {
    it('should return 1.0 for no type errors', () => {
      expect(calculateTypeScore(0, 0)).toBe(1.0);
    });

    it('should penalize heavily for type errors', () => {
      expect(calculateTypeScore(5, 0)).toBe(0.5);
    });

    it('should penalize less for warnings', () => {
      expect(calculateTypeScore(0, 10)).toBe(0.8);
    });

    it('should combine errors and warnings', () => {
      expect(calculateTypeScore(3, 5)).toBe(0.6);
    });

    it('should not go below 0', () => {
      expect(calculateTypeScore(20, 20)).toBe(0);
    });
  });
  ...
  describe('calculateSecurityScore', () => {
    it('should return 0 for critical vulnerabilities', () => {
      expect(calculateSecurityScore({ critical: 1, high: 0, medium: 0, low: 0 })).toBe(0);
    });

    it('should penalize high vulnerabilities heavily', () => {
      expect(calculateSecurityScore({ critical: 0, high: 5, medium: 0, low: 0 })).toBe(0);
      expect(calculateSecurityScore({ critical: 0, high: 1, medium: 0, low: 0 })).toBe(0.8);
    });

    it('should penalize medium vulnerabilities moderately', () => {
      expect(calculateSecurityScore({ critical: 0, high: 0, medium: 10, low: 0 })).toBe(0.5);
    });

    it('should penalize low vulnerabilities lightly', () => {
      expect(calculateSecurityScore({ critical: 0, high: 0, medium: 0, low: 100 })).toBe(0);
    });
  });
...
describe('Criteria Parsing', () => {
  ...
  describe('createGateFromCriteria', () => {
    it('should create a gate with normalized weights', () => {
      const criteria = parseCriteriaJson('correctness:0.3,security:0.7');
