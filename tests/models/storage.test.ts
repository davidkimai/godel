...
describe('AgentStorage', () => {
  ...
  describe('CRUD Operations', () => {
    it('should create an agent', () => {
      const agent = createAgent({ model: 'test', task: 'Test task' });
    ...
    it('should update an agent', () => {
      ...
      expect(storage.get(agent.id)?.status).toBe(AgentStatus.RUNNING);
    });
    ...
    it('should list all agents', () => {
      const agent1 = createAgent({ model: 'test1', task: 'Task 1' });
      const agent2 = createAgent({ model: 'test2', task: 'Task 2' });
  ...
  describe('Status Indexing', () => {
    it('should find agents by status', () => {
      const agent1 = createAgent({ model: 'test', task: 'Task 1' });
      const agent2 = createAgent({ model: 'test', task: 'Task 2' });
      storage.create(agent1);
      storage.create(agent2);

      storage.update(agent1.id, { status: AgentStatus.RUNNING });
      storage.update(agent2.id, { status: AgentStatus.COMPLETED });

      const running = storage.findByStatus(AgentStatus.RUNNING);
      const completed = storage.findByStatus(AgentStatus.COMPLETED);
  ...
  describe('Swarm Indexing', () => {
    it('should find agents by swarm', () => {
      const agent1 = createAgent({ model: 'test', task: 'Task 1', swarmId: 'swarm-1' });
      const agent2 = createAgent({ model: 'test', task: 'Task 2', swarmId: 'swarm-1' });
      const agent3 = createAgent({ model: 'test', task: 'Task 3', swarmId: 'swarm-2' });
      storage.create(agent1);
      storage.create(agent2);
  ...
  describe('Parent/Child Indexing', () => {
    it('should find child agents of a parent', () => {
      const parent = createAgent({ model: 'test', task: 'Parent' });
      const child1 = createAgent({ model: 'test', task: 'Child 1', parentId: parent.id });
      const child2 = createAgent({ model: 'test', task: 'Child 2', parentId: parent.id });
...
describe('TaskStorage', () => {
  ...
  describe('CRUD Operations', () => {
    it('should create a task', () => {
      const task = createTask({ title: 'Test', description: 'Test task' });
  ...
  describe('Status Indexing', () => {
    it('should find tasks by status', () => {
      const task1 = createTask({ title: 'Task 1', description: 'Desc' });
      const task2 = createTask({ title: 'Task 2', description: 'Desc' });
      storage.create(task1);
      storage.create(task2);

      storage.update(task1.id, { status: TaskStatus.IN_PROGRESS });

      const inProgress = storage.findByStatus(TaskStatus.IN_PROGRESS);
      const pending = storage.findByStatus(TaskStatus.PENDING);
  ...
  describe('Assignee Indexing', () => {
    it('should find tasks by assignee', () => {
      const task1 = createTask({ title: 'Task 1', description: 'Desc', assigneeId: 'agent-1' });
      const task2 = createTask({ title: 'Task 2', description: 'Desc', assigneeId: 'agent-1' });
      const task3 = createTask({ title: 'Task 3', description: 'Desc', assigneeId: 'agent-2' });
      storage.create(task1);
      storage.create(task2);
  ...
  describe('Dependency Queries', () => {
    it('should find tasks that depend on a given task', () => {
      const task1 = createTask({ title: 'Task 1', description: 'Desc' });
      const task2 = createTask({ title: 'Task 2', description: 'Desc', dependsOn: [task1.id] });
      const task3 = createTask({ title: 'Task 3', description: 'Desc', dependsOn: [task1.id] });
      storage.create(task1);
      storage.create(task2);
      storage.create(task3);
...
describe('MemoryStore', () => {
  ...
  describe('Combined Storage', () => {
    it('should store agents, tasks, and events', () => {
      const agent = createAgent({ model: 'test', task: 'Test' });
